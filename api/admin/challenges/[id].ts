...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const idRaw = parseInt(req.query['id'] as string, 10);
  if (isNaN(idRaw)) return res.status(400).json({ error: 'Invalid id' });
  const id = idRaw;
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      // Use id = ANY($1) with a Postgres integer array — safe with neon's tagged templates
      if (keptCcIds.length > 0) {
        await sql`
          DELETE FROM coding_challenges
          WHERE challenge_id = ${id} AND NOT (id = ANY(${keptCcIds}))
        `;
      } else {
      ...
      if (keptQIds.length > 0) {
        await sql`
          ...
          WHERE challenge_id = ${id} AND NOT (id = ANY(${keptQIds}))
        `;
      } else {
